# Perintah untuk membuat Docker image dari Dockerfile
docker build -t item-app:v1 .

# Melihat daftar image di lokal
docker images

# Mengubah nama image agar sesuai dengan format GitHub Container Registry
docker tag item-app:v1 ghcr.io/aldaaaaa/a433-microservices/item-app:v1

# Login ke GitHub Container Registry dengan personal access token
echo $GHCR_TOKEN | docker login ghcr.io -u aldaaaaa --password-stdin

# Mengunggah image ke GitHub Container Registry
docker push ghcr.io/aldaaaaa/a433-microservices/item-app:v1